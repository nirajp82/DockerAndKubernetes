...
The `docker ps` command provides a snapshot of all your currently running containers. It offers a quick overview of the containers' names, IDs, statuses, and resource usage.

### 2. `docker ps -a`: List All Containers (Stopped and Running)
Adding the `-a` flag expands the scope to include all containers, both running and stopped. This command gives you a comprehensive view of your container ecosystem.

### 3. `docker ps -a -q`: List Only Container IDs
For scripting and automation purposes, you can use `docker ps -a -q` to extract only the IDs of all containers. This minimalist output is handy when you need to pass container IDs as parameters to other commands.

### 4. `docker stop container-id`: Stop a Running Container
To gracefully stop a running container, use `docker stop` followed by the container's ID. This command sends a SIGTERM signal to the container, allowing it to shut down gracefully.

### 5. `docker start container-id`: Start a Stopped Container
...
### 6. `docker restart container-id`: Restart a Running Container
...
### 7. `docker port container-id`: List Port Mappings of a Container
...
### 8. `docker rm container-id` or `name`: Remove a Stopped Container
...
### 9. `docker rm -f container-id` or `name`: Remove a Running Container Forcefully
...
### 10. `docker pull image-info`: Pull an Image from Docker Hub
...
### 11. `docker pull nirajp82/my_first_docker_image`: Pull a Specific Image from Docker Hub
...
### 13. `docker login -u username -p password`
...
### 15. `docker top container-id or name`
...
### 17. `docker container run -d image_name`
...
### 20. `docker logs <container id> --follow`
...
### 21. `docker logs <container id> --since`
...
### 22. `docker logs <container id> --tail`
...
### 23. `docker logs <container id> --timestamps`
...
### 24. `docker logs <container id> --details`
...
### 25. `docker logs <container id> --until`
